...
Single-file, video-only OPAtom input is also supported for the exact operational-pattern label `060e2b34040101010d01020110000000`. It requires the same local, untrimmed package graph, one direct AVC descriptor with LinkedTrackID, one essence-container data set, and one frame-wrapped essence element per indexed edit unit. The index must have a single video DeltaEntry at offset zero, without slices. Separate audio files, Avid/AAF assembly, clip wrapping, encryption, and other OPAtom label variants are not supported.

A narrow compatibility path handles the Doremi LibMedia layout found in `h264_vid.mxf` and `dcinema-h264.mxf`. These files declare two container labels, legacy AVC `060e2b34040101020d01030102106001` and generic `060e2b34040101030d010301027f0100`, despite the OPAtom label. Their MPEGVideoDescriptor leaves PictureEssenceCoding zero and incorrectly puts High 10 Intra label `060e2b340401010a0401020201322001` in Codec. Only this combination enables compatibility; the payload must actually be progressive, 8-bit 4:2:0 Main-profile AVC. This is producer compatibility, not general OPAtom conformance or acceptance of arbitrary zero coding labels.

The Doremi path preserves the temporal-index mapping but validates the observed positive GOP distances separately from standard ST 381-3 backward key offsets. Its `0xc0` access-point flags do not distinguish the initial IDR from later non-IDR recovery pictures. **Only the initial IDR is exposed as a key packet.** `getKeyPacket(t)` returns frame 0 for every nonnegative timestamp, and `getNextKeyPacket()` returns null. Cold decoded seeks therefore read and decode from the beginning, with cost proportional to the target position; this is not sparse decoded seeking. Non-IDR recovery pictures remain delta packets even where a browser happens to accept them as restart points. The first full key read/configuration validates SPS/PPS/IDR, and full reads validate repeated parameter sets at recovery pictures against the first pair. Metadata-only reads do not inspect payloads. The OP1a closed-GOP constraints are unchanged.
